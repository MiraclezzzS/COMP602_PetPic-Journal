...
MeScroll.prototype.extendUpScroll = function(optUp) {
	...
	MeScroll.extend(optUp, {
		...
		page: {
			...
			size: 10, // 每页数据的数量
			time: null // 加载第一页数据服务器返回的时间; 防止User翻页时,后台Add了数据从而导致下一页数据重复;
		},
...
MeScroll.prototype.endUpScroll = function(isShowNoMore) {
	if (isShowNoMore != null) { // isShowNoMore=null,不处理下拉status,下拉刷新的时候调用
		if (isShowNoMore) {
...
MeScroll.prototype.setBounce = function(isBounce) {
	...
	if (isBounce === false) {
		...
		// 需禁止window的touchmove事件才能effective 阻止bounce
