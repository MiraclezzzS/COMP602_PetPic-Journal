<template>
	<div class="addEdit-block" :style='{"padding":"30px","background":"url(http://codegen.caihongy.cn/20220730/4b6f6d88fde34446904f3e1cd2807e39.png) center center/cover fixed no-repeat","height":"calc(100vh - 120px)"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="Photo Title" prop="zhaopianbiaoti">
					<el-input v-model="ruleForm.zhaopianbiaoti" placeholder="Photo Title" clearable  :readonly="ro.zhaopianbiaoti"></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="Photo description" prop="zhaopianmiaoshu">
					<editor 
						...
						v-model="ruleForm.zhaopianmiaoshu" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else-if="ruleForm.zhaopianmiaoshu" label="Photo description" prop="zhaopianmiaoshu">
                    <span :style='{"fontSize":"14px","lineHeight":"1.5","color":"#000","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.zhaopianmiaoshu"></span>
                </el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.zhaopianmiaoshu = this.ruleForm.zhaopianmiaoshu.replace(reg,'../../../ssm85bqd1pj/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `chongwuxiangce/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.chongwuxiangceCrossAddOrUpdateFlag = false;
						 this.parent.search();
