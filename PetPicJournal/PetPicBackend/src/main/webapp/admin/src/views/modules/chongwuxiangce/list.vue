		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"margin":"40px 0","display":"flex"}'>
					<el-button :style='{"border":"1px solid #4662A1","cursor":"pointer","padding":"0 24px","boxShadow":"0 2px 0 0 #97C9D6","margin":"0 10px 0 0","outline":"none","color":"#4662A1","borderRadius":"0","background":"#fff","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('chongwuxiangce','Add')" type="success" @click="addOrUpdateHandler()">Add</el-button>
					<el-button :style='{"border":"1px solid #4662A1","cursor":"pointer","padding":"0 24px","boxShadow":"0 2px 0 0 #97C9D6","margin":"0 10px 0 0","outline":"none","color":"#4662A1","borderRadius":"0","background":"#fff","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('chongwuxiangce','Delete')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">Delete</el-button>


					<el-button :style='{"border":"1px solid #4662A1","cursor":"pointer","padding":"0 24px","boxShadow":"0 2px 0 0 #97C9D6","margin":"0 10px 0 0","outline":"none","color":"#4662A1","borderRadius":"0","background":"#fff","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('chongwuxiangce','打印')" type="success" @click="printJson">打印</el-button>
			...
			<!-- <div> -->
				<el-table class="tables"
					...
					v-if="isAuth('chongwuxiangce','View')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="Operate">
						<template slot-scope="scope">
							<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 6px 0","outline":"none","color":"#fff","borderRadius":"30px","background":"#4662A1","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('chongwuxiangce','View')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">Details</el-button>
							<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 6px 0","outline":"none","color":"#fff","borderRadius":"30px","background":"#4662A1","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('chongwuxiangce','Modify')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">Modify</el-button>


							<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 6px 0","outline":"none","color":"#fff","borderRadius":"30px","background":"#4662A1","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('chongwuxiangce','View comments')" type="primary" size="mini" @click="disscussListHandler(scope.row.id)">View comments</el-button>



							<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 6px 0","outline":"none","color":"#fff","borderRadius":"30px","background":"#4662A1","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('chongwuxiangce','Delete') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">Delete</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
...
export default {
  ...
  methods: {
	...
	this.$router.push({path:'/discusschongwuxiangce',query:{refid:id}});
    },
    // 下载
    download(file){
      window.open(`${file}`)
    ...
    deleteHandler(id) {
      ...
      this.$confirm(`Confirm to proceed[${id ? "Delete" : "Batch Delete"}]Operate?`, "提示", {
        confirmButtonText: "Confirm",
        cancelButtonText: "cancel",
        type: "warning"
      }).then(() => {
        this.$http({
          url: "chongwuxiangce/delete",
          method: "post",
          data: ids
        }).then(({ data }) => {
          if (data && data.code === 0) {
