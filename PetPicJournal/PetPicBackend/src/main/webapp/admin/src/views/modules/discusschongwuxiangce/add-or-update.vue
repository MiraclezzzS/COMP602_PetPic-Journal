	...
	<div class="addEdit-block" :style='{"padding":"30px","background":"url(http://codegen.caihongy.cn/20220730/4b6f6d88fde34446904f3e1cd2807e39.png) center center/cover fixed no-repeat","height":"calc(100vh - 120px)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="Comment content" prop="content">
					<el-input
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else-if="ruleForm.content" label="Comment content" prop="content">
					<span :style='{"fontSize":"14px","lineHeight":"1.5","color":"#000","fontWeight":"500","display":"inline-block"}'>{{ruleForm.content}}</span>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="Reply content" prop="reply">
					<el-input
					  style="min-width: 200px; max-width: 600px;"
					  type="textarea"
					  :rows="8"
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else-if="ruleForm.reply" label="Reply content" prop="reply">
...
export default {
	data() {
		...
		return {
			...
			rules: {
				refid: [
					{ required: true, message: '关联表id not null', trigger: 'blur' },
				],
				userid: [
					{ required: true, message: 'Userid not null', trigger: 'blur' },
				],
				avatarurl: [
				],
				nickname: [
				...
				content: [
					{ required: true, message: 'Comment content not null', trigger: 'blur' },
				],
				reply: [
				],
			}
    ...
    info(id) {
      this.$http({
        url: `discusschongwuxiangce/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `discusschongwuxiangce/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.discusschongwuxiangceCrossAddOrUpdateFlag = false;
						 this.parent.search();
