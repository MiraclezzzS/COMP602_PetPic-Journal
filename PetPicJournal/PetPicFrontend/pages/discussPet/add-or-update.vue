<template>
<view class="content">
	<view :style='{"minHeight":"100vh","padding":"0px 0 80rpx","borderColor":"white","background":"#fff","borderWidth":"0px 0 0","width":"100%","position":"relative","borderStyle":"dashed","height":"auto"}'>
		<form :style='{"width":"100%","padding":"60rpx 40rpx","background":"none","height":"auto"}' class="app-update-pv">
			
			<!-- ${location} -->
 

			<view :style='{"padding":"0 20rpx 0px","margin":"0 0 20rpx","borderColor":"white","borderRadius":"0px","flexWrap":"wrap","borderWidth":"2rpx","background":"linear-gradient(270deg, white 0%, rgb(255 235 59 / 16%) 100%)","display":"flex","width":"100%","lineHeight":"100rpx","borderStyle":"solid","height":"auto"}' class="">
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				res = await this.$api.info(`discusspetimage`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusspetimage`, params);
					if (res.data.total >= crossoptnum) {
						...
						if(this.ruleForm.id){
							await this.$api.update(`discusspetimage`, this.ruleForm);
						}else{
							await this.$api.add(`discusspetimage`, this.ruleForm);
						}
						uni.setStorageSync('discusspetimage',true);
